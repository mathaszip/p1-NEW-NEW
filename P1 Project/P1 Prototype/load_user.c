...
int main(void) {
    ...
    groceries_list eliminated[MAX_STORES];
    groceries_list *ptr4 = eliminated;

    double sumArray[MAX];
    double *ptr3 = sumArray;

    ptr = open_files(); //first grocery list
    ptr1 = open_stores(); //
    ...
    //ptr4 = eliminate_groceries(user, ptr1, ptr, numberOfStores);

    //ptr3 = sumOfProducts(ptr4, numberOfStores);
...
/*double * sumOfProducts(groceries_list groceries_list[], int num) {
    double sum;
    ...
    for (int i = 0; i < num; i++) {
        ...
        for (int k = 0; k < MAX; k++) {
            ...
            valueinarray(userproducts[k], groceries_list[i].name);
...
double * sumOfProducts(groceries_list groceries_list[], int num) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        for (int k = 0; k < MAX; k++) {
            ...
            if(valueinarray(k, groceries_list[i].name)) {
                sum += groceries_list[i].cost[k];
            }
